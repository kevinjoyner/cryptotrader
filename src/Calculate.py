...
class Calculators:
    ...
    def stats_report(self, datehour, prices_df):
        ...
        for symbol in prices_df.columns:
            if prices_df[symbol][nineteen_hours_ago] and prices_df[symbol][hour_ago]:
                value = {}
                value['symbol'] = symbol
                value['recent_std_dev'] = np.std(
                    prices_df[symbol][nineteen_hours_ago:hour_ago].apply(float)
                )
